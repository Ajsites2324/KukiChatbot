...
async def addchat(_, message): 
    ...
    if not is_kuki:
        ...
        await message.reply_text(f"✅ | Successfully\nElisha Chatbot of this Group is set to @{message.chat.username}\n Requested by [{message.from_user.first_name}](tg://user?id={message.from_user.id})\n© @AJEET_BOTS")
    else:
        await message.reply_text(f"Already Setup ELISHA Chatbot of this Group Is @{message.chat.username}")
...
async def rmchat(_, message): 
    ...
    is_kuki = kuki.find_one({"chat_id": message.chat.id})
    if not is_kuki:
        await message.reply_text("Already Elisha ChatBot Disable baby")
    else:
        kuki.delete_one({"chat_id": message.chat.id})
        await message.reply_text("✅ | Elisha Chatbot is disable baby!")
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here",
                url=f"t.me/elishamachinebot?start")]])
        await message.reply("Contact me in PM",
                            reply_markup=buttons)
        
    else:
        buttons = [[InlineKeyboardButton("Support", url="https://t.me/about_ajeet"),
                    InlineKeyboardButton("Channel", url="https://t.me/ajeet_bots"),
                    InlineKeyboardButton("Repo", url="https://github.com/Ajsites2324/kukichatbot")
                    ]]
        Photo = "https://te.legra.ph/file/5df347adec4f961ce8ec3.jpg"
        await message.reply_photo(Photo, caption=f"Hello [{message.from_user.first_name}](tg://user?id={message.from_user.id}), Machine Learning Chat Bot that can talk about any topic in any language\n /help - Help Commands\n Powered By @AJEET_BOTS", reply_markup=InlineKeyboardMarkup(buttons))
...
async def help(client, message):
    ...
    if message.chat.type != "private":
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton(text="Click here",
                url=f"t.me/elishamachinebot?start=help_")]])
        await message.reply("Contact me in PM",
                            reply_markup=buttons)
        
    else:    
        await message.reply_text("/start - Start The Bot\n/chat - Send a message to this bot\n/setupchat - Active Elisha Chatbot In Group\n/removechat - Disable Elisha Chatbot In Group")
